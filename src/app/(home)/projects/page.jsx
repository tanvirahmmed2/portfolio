    ...
    <div className="min-h-screen py-24 px-6 sm:px-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10 space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 border-b pb-8">
          ...
          <div className="relative w-full md:w-80">
            <input
              ...
              className="w-full border rounded-xl px-4 py-2.5 pl-10 text-xs text-white focus:outline-none focus:border-violet-500 transition-all"
            />
            <svg className="absolute left-3 top-3 w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-pulse">
            <div className="h-64 rounded-3xl"></div>
            <div className="h-64 rounded-3xl"></div>
            <div className="h-64 rounded-3xl"></div>
          </div>
        ) : filteredProjects.length === 0 ? (
          <div className="p-12 text-center border rounded-3xl space-y-2">
            <p className="text-sm">No matching projects found.</p>
            <p className="text-xs">Try searching for alternative keywords or tools.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {filteredProjects.map((project) => (
              <div
                ...
                className="group border rounded-3xl overflow-hidden transition-all duration-300 flex flex-col justify-between cursor-pointer"
              >
                <div>
                  {/* Cover */}
      ...
      {selectedProject && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm">
          <div className="border rounded-3xl w-full max-w-2xl overflow-hidden max-h-[85vh] flex flex-col">
            ...
            <div className="relative aspect-video w-full border-b">
              ...
              <button
                ...
                className="absolute top-4 right-4 p-2 bg-black/60 border rounded-full hover:text-white transition-all cursor-pointer"
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
              ...
              {selectedProject.skills && selectedProject.skills.length > 0 && (
                <div className="space-y-2">
                  <span className="text-[9px] font-bold uppercase tracking-widest block">Technologies Used</span>
                  <div className="flex flex-wrap gap-2">
                    {selectedProject.skills.map((skill) => (
                      <span
                        key={skill.id}
                        className="text-[9px] font-bold border px-2.5 py-1 rounded-lg"
                      >
                        {skill.name}
                      </span>
                    ))}
              ...
              <div className="pt-4 border-t flex flex-wrap items-center gap-4">
                ...
                {selectedProject.github_url && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-5 py-2.5 rounded-xl text-xs font-bold border hover:text-white transition-all cursor-pointer"
                  >
