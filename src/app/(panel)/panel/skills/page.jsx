    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-400 text-sm mt-1">Manage core competencies, category listings, and proficiencies.</p>
        </div>
        <button
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-44 bg-neutral-900/40 border border-neutral-800/80 rounded-2xl p-6 flex flex-col justify-between animate-pulse">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 bg-neutral-800 rounded-xl"></div>
                <div className="space-y-2 flex-1">
                  <div className="h-4 bg-neutral-800 rounded-md w-2/3"></div>
                  <div className="h-3 bg-neutral-800 rounded-md w-1/3"></div>
                </div>
              </div>
              <div className="space-y-2">
                <div className="h-2 bg-neutral-800 rounded-full w-full"></div>
                <div className="h-3 bg-neutral-800 rounded-md w-10 ml-auto"></div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="text-center py-20 border border-dashed border-neutral-800/80 rounded-2xl bg-neutral-900/10">
          ...
          <p className="text-neutral-400 font-medium">No skills found in this category.</p>
          <button
            onClick={handleOpenCreateModal}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSkills.map((skill) => (
            <div
              key={skill.id}
              className="group bg-neutral-900/40 hover:bg-neutral-900/70 border border-neutral-800/80 hover:border-violet-500/20 rounded-2xl p-6 flex flex-col justify-between hover:-translate-y-0.5 transition-all duration-300 shadow-sm relative overflow-hidden"
            >
                ...
                <div className="flex items-start gap-4">
                  ...
                  {skill.image ? (
                    <img
                      src={skill.image}
                      alt={skill.name}
                      className="w-12 h-12 rounded-xl object-contain bg-neutral-900 border border-neutral-800/80 p-1"
                    />
                  ) : (
                    <div className="w-12 h-12 rounded-xl bg-neutral-800 flex items-center justify-center font-bold text-neutral-300 border border-neutral-700/60 uppercase">
                      {skill.name[0]}
                    </div>
                  )}
                  <div className="min-w-0">
                    <h3 className="font-bold text-white group-hover:text-violet-400 transition-colors truncate">{skill.name}</h3>
                    <span className="inline-block px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-neutral-800/80 border border-neutral-700/50 text-neutral-400 rounded-full mt-1.5">
                      {skill.category}
                    </span>
                  </div>
                </div>
              ...
              <div className="mt-6 relative z-10">
                <div className="flex justify-between items-center text-xs text-neutral-500 mb-1.5">
                  ...
                  <span className="font-bold text-neutral-300">{skill.proficiency}%</span>
                </div>
                ...
                <div className="flex justify-between items-center text-[10px] text-neutral-600 mt-2 font-medium">
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-lg p-6 shadow-2xl relative overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-neutral-400 uppercase tracking-wide">Skill Name</label>
                <input
                  type="text"
                  value={name}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <select
                    ...
                    className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition-all duration-200"
                  >
                    {categories.map((cat) => (
                ...
                {/* Display Order */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold text-neutral-400 uppercase tracking-wide">Display Order</label>
                  <input
                    type="number"
                    value={displayOrder}
              ...
              <div className="flex flex-col gap-1.5">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-neutral-400 uppercase tracking-wide">Proficiency</label>
                ...
                <input
                  ...
                  className="w-full accent-violet-500 h-2 bg-neutral-950 border border-neutral-800 rounded-lg cursor-pointer"
                />
              </div>

              {/* Featured Toggle */}
              <div className="flex items-center gap-3 bg-neutral-950/40 border border-neutral-800/80 p-3.5 rounded-xl">
                <input
                  type="checkbox"
                  id="featured"
                  checked={isFeatured}
                ...
                />
                <label htmlFor="featured" className="text-xs font-bold text-neutral-300 uppercase tracking-wide cursor-pointer select-none">
                  Pin to Featured Skills
                </label>
              </div>

              {/* Icon Uploader */}
              <div className="flex flex-col gap-2 bg-neutral-950/20 border border-neutral-800/85 p-4 rounded-xl">
                <label className="text-xs font-bold text-neutral-400 uppercase tracking-wide">Skill Icon / Logo</label>
                
                <div className="flex items-center gap-4 mt-1">
                  {/* Preview Container */}
                  <div className="w-16 h-16 rounded-xl bg-neutral-950 border border-neutral-800/80 flex items-center justify-center relative overflow-hidden flex-shrink-0">
                    {uploadingImage ? (
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-neutral-500 mt-1.5">PNG, SVG or JPEG. Recommended size under 1MB.</p>
                  </div>
                </div>
              </div>
